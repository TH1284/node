...
def insertNodelist(ip, port):
    try:
        pd.read_sql_query("insert into nodelist (ip, port) values ('{}', '{}')".format(ip, port), engine)
    except:
        pass
...
def readPagingBlockchain(g_bcTableName, start, end, mode='internal'):
    ...
    try:
        ...
        if start > len(importedBlockchain) or start < 1:
            ...
            if end > len(importedBlockchain):
                end = len(importedBlockchain)
...
def getCountedTxData(count):
    strTxData = ''
    importedTx = readPagingTx(g_txTableName, 1, int(count))
